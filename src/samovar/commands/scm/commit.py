...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if len(args) > 0 and kwargs.get('message') is not None:
            self.ui.error('Abort: cannot provide the commit message both as a switch and a positional argument')
    ...
    def _get_commiter(self, repo, user):
        if user is not None:
            return user
        # Try in repository
        config_path = 'workspaces.%s.repositories.%s.committer' % (self.config.active, repo.name)
        repo_commiter = self.config.get(config_path, None)
        if repo_commiter is not None:
            return repo_commiter
        # Try in workspace
        return self.config.get('workspaces.%s.committer' % self.config.active, None)
