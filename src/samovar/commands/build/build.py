...
class Command(BaseCommand):
    ...
    def save_log(self, project, data):
        logpath = os.path.join(self.config.logdir, project.path.replace(self.config.active_path, '').strip('\\/'), project.id + '.log')
    ...
    def handle(self, platform, configuration, build_number, no_deps, delete, *args, **kwargs):
        ...
        if no_deps:
            ...
            for repo in self.config.all_repositories:
                ...
                if repo.name not in (self.config.options.skipped_repositories if self.config.options.skipped_repositories is not None else []):
                    b = utils.load_build(repo)
                    all_repos[b.name] = b
